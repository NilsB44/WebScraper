...
class TestAnalysis(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_search_urls_known_sites(self):
    ...
    async def test_get_search_urls_unknown_site(self):
        # Mock generate_content_safe to return a custom response for an unknown site
        mock_response = MagicMock()
        mock_response.parsed = SearchURLGenerator(search_pages=[
            SearchPageSource(site_name="unknown.site", search_url="https://unknown.site/search?q=XTZ")
        ])
